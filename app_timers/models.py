...
from django.db import models
from django.conf import settings  # CustomUser modelini almak için
...
class TimerSession(models.Model):
    user = models.OneToOneField(CustomUser, on_delete=models.CASCADE,null=False)  # Unique session per user
    timer_blocks = models.TextField(null=True, blank=True)

    def __str__(self):
        return f'TimerSession {self.user} - Blocks: {self.timer_blocks}'
