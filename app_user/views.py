...
class LoginView(APIView):
    ...
    API to log a user into their account.
    ...
    - 📤 **Outputs:**
        ...
        - ❌ **Failed Login:**
            ...
            - If email verification is not completed:
                - ❌ Status: 401 Unauthorized
                - 📄 Response JSON: `{"error": "Email not verified."}`
    """
    ...
    def post(self, request):
        ...
        if user:
            if user.is_mail_verified:
                login(request, user)
                session_id = request.session.session_key  # Retrieve session ID
                token, created = Token.objects.get_or_create(user=user)  # Get or create user token
                return Response({'status': "login successfully", "sessionid": session_id, 'token': token.key},
